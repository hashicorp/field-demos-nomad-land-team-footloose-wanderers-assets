...
Add-Content -Path "${Base_Directory}/nomad/nomad_client.hcl" -Encoding ascii -Value @"
name        = "${x}"
region      = "${r}"
datacenter  = "${d}"
...
"@
sc.exe create "Nomad" binPath= "c:/windows/system32/nomad.exe agent -config ${Base_Directory}/nomad/" DisplayName= "HashiCorp Nomad" start= auto
sc.exe start Nomad
